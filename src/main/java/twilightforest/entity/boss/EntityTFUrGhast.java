...
public class EntityTFUrGhast extends EntityTFTowerGhast {
        ...
        if (!world.isRemote) {
            ...
            if (this.isInTantrum()) {
                TwilightForestMod.proxy.spawnParticle(TFParticleType.BOSS_TEAR,
                        this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width * 0.75D,
                        this.posY + this.rand.nextDouble() * (double) this.height * 0.5D,
                        this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width * 0.75D,
                        0, 0, 0
                );
            }
    ...
    public boolean isEntityInvulnerable(DamageSource src) {
        return src == DamageSource.IN_WALL || src == DamageSource.IN_FIRE || src == DamageSource.ON_FIRE || super.isEntityInvulnerable(src);
    }
