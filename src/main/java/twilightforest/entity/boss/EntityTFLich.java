...
import twilightforest.entity.ai.EntityAITFLichPopMobs;
import twilightforest.entity.ai.EntityAITFLichShadows;
import twilightforest.entity.ai.EntityAITFLichShootDagger;
import twilightforest.entity.ai.EntityAITFLichWatchTarget;
...
public class EntityTFLich extends EntityMob {
    ...
    public static final Set<Class<? extends Entity>> POPPABLE = ImmutableSet.of(EntitySkeleton.class, EntityZombie.class, EntityEnderman.class, EntitySpider.class, EntityCreeper.class, EntityTFSwarmSpider.class);
    ...
    protected void initEntityAI() {
        ...
        this.tasks.addTask(1, new EntityAITFLichWatchTarget(this));
        this.tasks.addTask(1, new EntityAITFLichPopMobs(this));
        this.tasks.addTask(2, new EntityAITFLichShadows(this));
        this.tasks.addTask(3, new EntityAITFLichMinions(this));
        this.tasks.addTask(4, new EntityAITFLichShootDagger(this));
        this.tasks.addTask(5, new EntityAIAttackMelee(this, 0.75D, true) {
            @Override
    ...
    public void onLivingUpdate() {
        ...
        if (!world.isRemote) {
            if (this.getPhase() == 1) {
                bossInfo.setPercent((float) (getShieldStrength()) / (float) (INITIAL_SHIELD_STRENGTH));
            } else {
     ...
     */
    @Nullable
    public Vec3d findVecInLOSOf(@Nullable Entity targetEntity) {
    ...
    public void makeRedMagicTrail(double srcX, double srcY, double srcZ, double destX, double destY, double destZ) {
        int particles = 60;
        for (int i = 0; i < particles; i++) {
            double trailFactor = i / (particles - 1.0D);
            float f = 1.0F;
