    ...
    public void onUpdate() {
        ...
        if (this.world.isRemote) {
            ...
            if (this.isGroundAttackCharge()) {
                this.clientSideChargeAnimation = MathHelper.clamp(this.clientSideChargeAnimation + (1.0F / ((float) dataManager.get(GROUND_CHARGE)) * 6.0F), 0.0F, 6.0F);
                ...
                if (groundSmashState) {
                    ...
                    for (int i = 0; i < 80; i++) {
                        double cx = getPosition().getX() + world.rand.nextFloat() * 10.0F - 5.0F;
                        double cy = getEntityBoundingBox().minY + 0.1F + world.rand.nextFloat() * 0.3F;
                        double cz = getPosition().getZ() + world.rand.nextFloat() * 10.0F - 5.0F;

                        world.spawnParticle(EnumParticleTypes.BLOCK_CRACK, cx, cy, cz, 0.0D, 0.0D, 0.0D, stateId);
                    }
