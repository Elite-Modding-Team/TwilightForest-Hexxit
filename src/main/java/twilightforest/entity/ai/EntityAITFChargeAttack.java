...
public class EntityAITFChargeAttack extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (this.chargeTarget == null) {
            ...
            double minoshroomBonus = this.charger instanceof EntityTFMinoshroom ? 9.0D : 0.0D;
            if (distance < MIN_RANGE_SQ + minoshroomBonus || distance > MAX_RANGE_SQ + minoshroomBonus) {
                return false;
            } else if (!this.charger.onGround) {
                return false;
            } else {
                Vec3d chargePos = findChargePoint(charger, chargeTarget, 2.1D);
    ...
    public void updateTask() {
        ...
        double rangeSq = this.charger instanceof EntityTFMinoshroom ? 5.0D : this.charger.width * 2.0F * this.charger.width * 2.0F + this.chargeTarget.width;
